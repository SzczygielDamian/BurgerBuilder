...
const OrderSummary: React.FC<OrderSummaryProps> = ({ burgerState, clickOrder }) => {

    const ingredientSummry = Object.keys(burgerState.burger).map(ingKey => {
        return <li key={ingKey}><span>{ingKey}</span>: {burgerState.burger[ingKey]}</li>
    })
    ...
    return (  
        <>
            ...
            <p><strong>Total Price: {burgerState.totalPrice.toFixed(2)}</strong></p>
